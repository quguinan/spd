   ...
   <base href="<%=basePath%>">
	...
	<script type="text/javascript">  
		...
		$(function(){
			$("#dg_dtl").datagrid({
			    ...
			    columns:[[
							...
							{field:'qty', title:'申请退库数量',width:100,align:'center',editor : 'numberbox'},
							{field:'realqty', title:'实际退库数量',width:100,align:'center'}, 
			...
			$('#dg_history_dtl').datagrid({ 
			    ...
			    columns:[[
							...
							{field:'goodsname', title:'货品名称',width:120,align:'center' },
							{field:'spec', title:'规格',width:120,align:'center' },
							{field:'qty', title:'申请退库数量',width:100,align:'center'},
							{field:'realqty', title:'实际退库数量',width:100,align:'center'}, 
							{field:'unitname', title:'单位',align:'center' },
							{field:'memo', title:'备注',align:'center' },
			              ]],
		...
		function addDoc(){
			$("#fm_doc").form("clear");
			$("#billtype").val("KSSQ");
			$("#billname").textbox('setValue',"科室退库");
		...
		function openStockSelect(){
			...
			$('#winStockSelect').window({
				//href: '${pageContext.request.contextPath}/cms/common/goodsStock2SelectDtl?deptid='+deptid,
				href: '${pageContext.request.contextPath}/cms/common/goodsStock2SelectSum?deptid='+deptid,
			});
			$("#winStockSelect").window("open");
		};
		//新增细单
		function addDtl(deptid,goodsid,goodsdtlid){
			// 校验批次号是否已经被选择
			var rows=$('#dg_dtl').datagrid('getRows');
			for (var i = 0; i < rows.length; i++) {
				if(rows[i].goodsdtlid==goodsdtlid){
					$.messager.show({title: '提示',msg: "不能重复选择同一条库存!",showType:'show'});
					return;
				}
			} 
			//
			$.post(WEB_ROOT+'/cms/goods/stock2/stock2_refund/getGoodsStock2Select', {deptid:deptid,goodsid:goodsid, goodsdtlid:goodsdtlid}, 
				function(result) {
					if(result.success){ 
						...
						edit($("#dg_dtl"));
						//endEdit($("#dg_dtl"));//不允许更改退货数量，只能按批号全退
					}else{
		...
		function save(){
			...
			$('#fm_doc').form('submit', {    
			    url:WEB_ROOT+'/cms/goods/stock2/stock2_refund/save',    
			    ...
			    success:function(data){   
			    	...
			    	$('#dg_dtl').datagrid('reload',WEB_ROOT+'/cms/goods/stock2/stock2_refund/gridDataDtl?docid='+result.docData.docid);
			    }    
			});  
		}
		//设置状态  作废or审核
		function setStatus(value){
			...
			$.post(WEB_ROOT+'/cms/goods/stock2/stock2_refund/setStatus', { docid : docid,status:status,value:value }, 
					function(result) {
						if(result.success){ 
							$('#fm_doc').form('load',result.docData);
							$('#dg_dtl').datagrid('reload',WEB_ROOT+'/cms/goods/stock2/stock2_refund/gridDataDtl?docid='+result.docData.docid);
		...
		function query(){
			...
			$('#dlg_history').dialog("open");
 			...
 			$('#dg_history_doc').datagrid('load',WEB_ROOT+'/cms/goods/stock2/stock2_refund/gridDataDocBycredate?'+$('#fmHistory').serialize()); 
		...
		function loadBill(){
			...
			if(row){
				$('#fm_doc').form('load',WEB_ROOT+'/cms/goods/stock2/stock2_refund/gridDataDocByid?docid='+row.docid);
		    	$('#dg_dtl').datagrid('load',WEB_ROOT+'/cms/goods/stock2/stock2_refund/gridDataDtl?docid='+row.docid);
		...
		function refund(){
			var docid=$("#docid").val();
			var status=$("#status").val();
			$.post(WEB_ROOT+'/cms/goods/stock2/stock2_refund/refund', { docid : docid,status:status }, 
					function(result) {
						if(result.success){ 
							$('#fm_doc').form('load',result.docData);
							$.messager.show({title: '提示',msg: result.msg,showType:'show'});
...
<body class="easyui-layout">
		...
		<form:form id="fm_doc" modelAttribute="form"  method="post">
			<div id="toolbar_doc" class="datagrid-toolbar">
		        <table cellpadding='1' cellspacing='1'>
					<tr>
						...
						<td><div class='btn-separator'></div></td>
						<td><a href='javascript:void(0);' class='easyui-linkbutton' iconCls='icon_tick' plain='true' onclick='refund()'>退库</a></td>
						<td><div class='btn-separator'></div></td>
		    ...
		    <table style="font-size:12;padding:3 0 0 0">
		    	<tr>
		    		...
		    		<td>单据类型</td><td><form:input id="billname" path="billname" type="text" class="easyui-textbox"  data-options="required:false,disabled:true" readOnly="true" value="科室退库" /></td>
	...
	<div id="winStockSelect" class="easyui-window" title="选择二级库存货品" style="width:600px;height:430px"   
	...
	<script type="text/javascript">
		...
		function getStock2Goodsid(deptid,goodsid,goodsdtlid){
			addDtl(deptid,goodsid,goodsdtlid);
		}
	</script>
	<!-- 库存货品  end -->
</body>
