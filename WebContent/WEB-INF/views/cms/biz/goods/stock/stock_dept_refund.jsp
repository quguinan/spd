	...
	<jsp:include page="../../../../inc/injs.jsp"/> 
		...
		$(function(){
			$("#dg_dtl").datagrid({
			    ...
			    columns:[[
							...
							{field:'realqty', title:'入库数量',width:100,align:'center',editor : 'numberbox'}, 
			...
			$('#dg_history_doc').datagrid({ 
			    ...
			    columns:[	 
					...
					pageList : [15,30,60] ,
					onClickRow: function (index, row) {
						$("#dg_history_dtl").datagrid({url:WEB_ROOT+'/cms/goods/stock/stock_dept_refund/gridDataDtl?docid='+row.docid});
					},
					onDblClickRow: function (index, row) {
						loadBill();
					},
		...
		function saveDtl(){
			...
			if ($("#dg_dtl").datagrid('getChanges').length) {
				...
				$.post(WEB_ROOT+'/cms/goods/stock/stock_dept_refund/saveDtl', { docid:docid,dataDtl:dataDtl }, 
						function(result) {
							if(result.success){ 
		...
		function setStatus(value){
			...
			$.post(WEB_ROOT+'/cms/goods/stock/stock_dept_refund/setStatus', { docid : docid,status:status }, 
					function(result) {
						if(result.success){ 
		...
		function instock(){
			var docid=$("#docid").val();
			var status=$("#status").val();
			$.post(WEB_ROOT+'/cms/goods/stock/stock_dept_refund/instock', { docid : docid,status:status }, 
					function(result) {
						if(result.success){ 
							$('#fm_doc').form('load',result.docData);
							$.messager.show({title: '提示',msg: result.msg,showType:'show'});
		...
		function query(){
			...
			$('#dlg_history').dialog("open");
 			...
 			$('#dg_history_doc').datagrid('load',WEB_ROOT+'/cms/goods/stock/stock_dept_refund/gridDataDocBycredate?'+$('#fmHistory').serialize()); 
		...
		function loadBill(){
			...
			if(row){
				$('#fm_doc').form('load',WEB_ROOT+'/cms/goods/stock/stock_dept_refund/gridDataDocByid?docid='+row.docid);
		    	$('#dg_dtl').datagrid('load',WEB_ROOT+'/cms/goods/stock/stock_dept_refund/gridDataDtl?docid='+row.docid);
